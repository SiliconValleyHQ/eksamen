 ...
 */
class Brett extends JFrame implements Serializable {

	/** Antall rader på brettet */
	private static final int rader = 8;

	/** Antall kolonner på brettet */
	private static final int kolonner = 8;
	 ...
	 * 		Raden som sjekkes
	 * @param kolonne
	 * 		Kolonnen som sjekkes
	 * @return True hvis posisjonen er lovlig, false hvis ikke
	 */
	private static boolean innenforRammene(int rad, int kolonne) {
		if (rad >= 0 && rad < rader &&
	...
	void plasserBrikkerForStart() {

		//Plasserer røde øverst
		for (int rad = 0; rad < 3; rad++) {
			for (int kolonne = 0; kolonne < 8; kolonne++) {
				if (getRute(rad, kolonne).getBackgroundColor() == Rute.bakgrunnsFarge.MORK) {
					getRute(rad, kolonne).setOkkupant(new Brikke(Color.RED, rad, kolonne));
				}
			}
		}
		...
		for (int rad = 5; rad < 8; rad++) {
			for (int kolonne = 0; kolonne < 8; kolonne++) {
				if (getRute(rad, kolonne).getBackgroundColor() == Rute.bakgrunnsFarge.MORK) {
					getRute(rad, kolonne).setOkkupant(new Brikke(Color.BLACK, rad, kolonne));
				}
			}
		}
	}
	 ...
	 */
	Vector<Rute> hentMuligeTrekk(Brikke brikke) {
		...
		if (Brett.innenforRammene(rad - 1, kolonne - 1) && brikkeFarge == Color.BLACK) {
			...
			else if (Brett.innenforRammene(rad - 2, kolonne - 2)) {
	 ...
	 *
	 * @param p
	 * 		Brikken som vi skal vise trekkene til
	 * @param uthev
	 * 		Stedene som skal merkes
	 */
	void setUthevningPaaMuligeTrekk(Brikke p, boolean uthev) {
	...
	boolean move(Rute fra, Rute til) {
