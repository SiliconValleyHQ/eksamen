...
public class DamSpill implements MouseListener, ActionListener, Serializable {
	...
	private DamSpill() {
		LagOgVisGUI(); //Viser grafikken

		naaverendeSpiller = Color.GREEN; //Definerer første spiller

		//antall brikker hver spiller starter med
		RodeBrikkerSomErIgjen = 12;
		svarteBrikkerSomErIgjen = 12;

		oppdaterStatus(); //hvor mange brikker som er igjen
	}

	/** Setter opp den grafiske visningen av spillet */
	private void LagOgVisGUI() {
		...
		vindu.getContentPane().setLayout(new BoxLayout(vindu.getContentPane(), BoxLayout.Y_AXIS));
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (valgt.erOkkupert() && valgtRute == null) {
			...
			valgtRute = valgt; //Endre uthevet rute
			valgtRute.setUtheving(true);
			brett.setUthevningPaaMuligeTrekk(valgtRute.getOkkupant(), true);
			...
			brett.setUthevningPaaMuligeTrekk(valgtRute.getOkkupant(), false);
			valgtRute = null;
		} else if (!valgt.erOkkupert() && valgtRute != null) { //Sjekker trekk
			...
			for (Rute valg : gamleMuligeTrekk) {
				if (valg.equals(valgt)) {
					...
					if (naaverendeSpiller == Color.GREEN) { //Se om første trekk er utført
						naaverendeSpiller = valgtRute.getOkkupant().getFarge();
					}

					trekk = brett.move(valgtRute, valgt); //lagre variablen uansett om noen brikke er flyttet eller ikke
					funn = true;//funn av trekk og og trekket er utført
				}
			}
			...
			if (funn) {
				...
				valgtRute.setUtheving(false); // Fjern uthevningen trekkene brikken kunne ta fra sin forrige posisjon
				for (Rute fjernUthevning : gamleMuligeTrekk)
					fjernUthevning.setUtheving(false);
				valgtRute = null;

				sluttTur();
				oppdaterStatus(); //Oppdaterer antall brikker

				String vinnerStr = vinner(); //Sjekk om trekket gjorde slik at noen vant
				if (vinnerStr != null) {
			...
			} else if (!funn) { //Forteller brukeren at trekket ikke er lov
	 ...
	 *
	 * @param brett
	 * 		Brettet som legges til panelet
	 * @param panel
	 * 		panelet
	 */
	private void leggBrettTilPanel(Brett brett, JPanel panel) {
		for (int i = 0; i < 8; i++) {
			for (int j = 0; j < 8; j++) {
